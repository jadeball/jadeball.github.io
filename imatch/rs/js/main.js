...
JKB.submit = {
	ajax_post : function (dir, action, param, callback)
	{
		var arr = dir.split('/');
		if(!arr[0]) 
		{
		...
		var url = '/jkb/' + arr[0] + '_' + arr[1] + '/' + action + '/x';
		jQuery.post(url,param,callback);
	},
	post : function (form, dir, action, btn_submit)
	{
		var arr = dir.split('/');
		if(!arr[0]) 
		{
		...
		var url = '/jkb/' + arr[0] + '_' + arr[1] + '/' + action + '/s';
		form.action = url;
		form.method = 'post';
	...
	go_dispose : function(dir, action, param)
	{
	   var arr = dir.split('/');
		if(!arr[0]) 
		{
	...
	ajax_load : function (load_id,dir, action, param, callback)
	{
		var arr = dir.split('/');
		if(!arr[0]) 
		{
		...
		{
			jQuery("#"+load_id).load(url, param,callback);
		}
	}
};
...
JKB.loader = {
	...
	{    
	    var ent_node_id = '';
		if(JKB.cache.ent_node_id) {ent_node_id = JKB.cache.ent_node_id;}
		this.showAjaxLoader();
		ajax_url = getAjaxWrapper('get_task_list_page',
            {type: JKB.cache.task_list_type,
             owner: JKB.cache.task_list_owner,
             priority: JKB.cache.task_list_priority,
             class_id:JKB.cache.task_list_class_id,
             status:JKB.cache.task_list_status,
             temp: JKB.cache.task_list_page,
             page: JKB.cache.page,
             s:encodeURIComponent(JKB.cache.s),
             domain_id:JKB.cache.task_list_domain,
             period:JKB.cache.curr_period,
             range:JKB.cache.date_range,
             m_page:m_page,
             ent_node_id:ent_node_id,
             search_status:JKB.cache.search_status});
...
//            if(JKB.cache.task_list_type=='site'){
//                getMonitorList();
//            }
		});
	},
	...
	loadTaskListData: function(period)
	{
		...
		switch (JKB.cache.task_list_page)
		{
			...
			case 'content_task_list':

				JKB.loader.loadContentTaskList();
				break;
			case 'api_task_list':
				JKB.loader.loadApiTaskList(period);
				break;
		}
	},
	loadContentTaskList: function()
	{
	
		JKB.loader.showAjaxLoader();
		var page = JKB.cache.page ? JKB.cache.page : '1';
		var domain_id = JKB.cache.content_domain_id ? JKB.cache.content_domain_id : '';
		var class_id = JKB.cache.content_class_id ? JKB.cache.content_class_id : '';
		var query = JKB.cache.s ? JKB.cache.s : ''; 
		var query_status = JKB.cache.search_status ? JKB.cache.search_status : '';
		
		JKB.submit.ajax_load('__div_task_list','content','load_task_list',{period:JKB.cache.curr_period,date_range:JKB.cache.date_range,page:page,domain_id:domain_id,class_id:class_id,query:encodeURIComponent(query),query_status:query_status},function(){
	        JKB.loader.hideAjaxLoader();
		});
		return false;
    },
    loadApiTaskList: function(period)
    {
    	JKB.loader.showAjaxLoader();
    	if(typeof period == 'undefined') period = '';
    	JKB.cache.curr_period = period;
    	$("#api_controll_date_range").children().each(function(){
    		if($(this).attr('time_type') == period){
    	...
    	var page = JKB.cache.page ? JKB.cache.page : '1';
		var domain_id = JKB.cache.api_domain_id ? JKB.cache.api_domain_id : '';
		var class_id = JKB.cache.api_class_id ? JKB.cache.api_class_id : '';
		var query = JKB.cache.s ? JKB.cache.s : ''; 
		var query_status = JKB.cache.search_status ? JKB.cache.search_status : '';
		var ent_node_id = JKB.cache.ent_node_id ? JKB.cache.ent_node_id : '';
		if(JKB.cache.search_status === 0) query_status='0';
		var submit_data = {
				period:JKB.cache.curr_period,
				date_range:JKB.cache.date_range,page:page,
				domain_id:domain_id,
				class_id:class_id,
				page:page,
    	...
    	JKB.submit.ajax_load('__div_task_list','api','load_task_list',submit_data,function(){
	        JKB.loader.hideAjaxLoader();
		});
    },
    filterTaskListData: function(form)
    {
//        var keyword = $(form).children("input").val();
//        var search_status = $(form).children("select").val();

        var keyword =$("#keywords").val();
        var search_status =$("#selectChange option:selected").val();
        JKB.cache.s = keyword;
        JKB.cache.search_status = search_status;
        JKB.cache.ent_node_id ='';
        if(JKB.cache.task_sort == 5)
		{
			this.loadContentTaskList();
	    }
		else
		{
			this.loadTaskListPage();
	...
	submitDateTimeRange: function(start_id, end_id)
	{
		...
		if (datetime_s == '' || datetime_e == '')
		{
			...
			{
				url = url.replace(/\?range=[ %:0-9a-z\,\-]*/g, '');
			}
			if(url.indexOf('&range=') !=-1)
			{
				url = url.replace(/\&range=[ %:0-9a-z\,\-]*/g, '');
			...
			if(is_more == 'more_select')
			{
				url = url + '&task=' + task_type + '&alert=' + alert_type + '&key=' + key_word;
			}
		}
		else
		{
        ...
        jQuery('#btn_clear').hide();
		window.location = url;
	},
    clearDateTimeRange: function (start_id, end_id)
    {   
		jQuery("#__datetime_start").val('');
		jQuery("#__datetime_end").val('');
		jQuery("#key_word").val('');
		jQuery("#task_type option").each(function(){
			if ($(this).text()=="所有")
			{
				$(this).attr("selected","true");
			}
		});
		jQuery("#alert_type option").each(function(){
			if ($(this).text()=="所有")
			{
				$(this).attr("selected","true");
			}
    ...
    fadeOutDateTimeRange: function()
    {
        jQuery('#datetime_range').fadeOut();
        return false;
    },
    fadeInDateTimeRange: function()
    {
        jQuery('#datetime_range').fadeIn();
        return false;
    },
	...
	onChangeProxyUser: function(user_id)
	{   
		var url = window.location.href;
		if(user_id){
			if(url.indexOf('user_id=') != -1)
			{
			   url = url.replace(/user_id=[0-9]+/g, 'user_id=' + user_id);
			}
			else
			{
				if (url.indexOf('?') != -1)
	...
	startTaskComp: function(obj)
	{
		...
		else
		{
			if(obj.checked)
			{
				window.location = '/adv_compare.php?task_ids=' + obj.value + '&type=periodcal';
			}
			else
			{
				jkbAlert(err, err);
    ...
    startTaskComps: function(obj)
    {
        var err = '很抱歉，对比项目数不能超过 2 个！';
        var errs='很抱歉，至少选择一个对比项目！';
        if (!obj)
        {
            jkbAlert(err, 'error');
            return;
	...
	startTaskDelete: function(obj)
	{
        ...
        JKB.submit.ajax_post('task/task_dispose','batch_user_task_delete',{task_ids:task_ids},function(data)
		{   
			if(data == '1')
			{
				JKB.loader.hideAjaxLoader();
				...
				/*
				split_task_id = task_ids.split(",");
				for(var i=0;i<split_task_id.length;i++){
					if(split_task_id[i] != "")
					{
            ...
            {   
                ...
                {   
                    ...
                    if(obj_data[split_task_id[i]] != "")
                    {
                        error_str += obj_data[split_task_id[i]] + "<br>";
                    }
                    else
                    {
                    	parent.JKB.loader.hideTask(split_task_id[i]);
	...
	toggleTaskPause: function(obj,type)
	{
		...
		task_ids = JKB.loader.getCheckedTasks();
        if(!task_ids){
            showSuccMsg('至少选择一个监控项目', '', '', {});
            JKB.loader.hideAjaxLoader();
            return false;
        }

		//var url = getAjaxWrapper('toggle_batch_task_pause', {task_ids:task_ids, type:type});
		JKB.submit.ajax_post('task/task_dispose','toggle_batch_task_pause',{task_ids:task_ids, type:type},function(data)
		{
			if(data == '1')
			{
				JKB.loader.hideAjaxLoader();
				...
				for(i=0;i<split_task_id.length;i++){
					if(split_task_id[i] != "")
					{
						...
						jQuery('#__task_pause_' + split_task_id[i]).css('display','inline-block');
				...
				if($('#task_num'))
				{
					var length = split_task_id.length;
					var stop_num = parseInt($('#task_num span:eq(1)').html());
					var open_num = parseInt($('#task_num span:eq(2)').html());
					$('#task_num span:eq(1)').html(stop_num - length);
					$('#task_num span:eq(2)').html(stop_num + length);
				}
			...
			else if(data == '3')
			{
				...
				for(i=0;i<split_task_id.length;i++){
					if(split_task_id[i] != "")
					{
                        ...
                        //alert(1);
						jQuery('#__task_open_' + split_task_id[i]).css('display','inline-block');
					}
				}
				//全景处理
				if($('#task_num'))
	...
	toggleUserReportStatus: function(report_id)
	{
		jQuery.post(getAjaxWrapper('toggle_user_report_status'), {report_id:report_id}, function(data)
		{
			if (data == '0')
			{
				jQuery('#__report_status_btn_' + report_id).html('<i class="table_handle_icon start"></i>');
			...
			{
				jQuery('#__report_status_btn_' + report_id).html('<i class="table_handle_icon pause"></i>');
	...
	loadThCondSelect: function()
	{
        ...
        if( metric != 4 ) 
            jQuery("#__th_cond_div, #__th_threshold_div").show();
        else
            jQuery("#__th_cond_div, #__th_threshold_div").hide();
...
//			$('#__newui_max_sel').remove();
		}
		
		if (cond_info['dev_title'] && cond_info['dev'])
		{
        ...
        if( !jQuery("#__th_dev_sel").is("select") ) {
            jQuery("#__th_dev_sel").empty();
        }

		if (cond_info['dev'])
		{
            var th_monitor_set = json_parse(JKB.cache.th_monitor_set);
            var th_monitor_set_html = '';        
            $.each(th_monitor_set, function(i, n){
                if( i == 3 && metric == 4 ) 
                    th_monitor_set_html += '';
                else
                    th_monitor_set_html += '<option value="'+i+'">'+n+'</option>';
            });
            jQuery("#__monitor_set").show();
            jQuery("#__th_monitor_set").html(th_monitor_set_html);
            $("#__th_monitor_input").html('<input type="text" value="" id="__th_monitor_count" name="th_monitor_count" class="mon_dist_val"> 个');
            this.switchMonitorCond();
		}
        else
        {

            jQuery("#__monitor_set").hide();
            jQuery("#__th_monitor_set").html('<option value="0"></option>');
        }
	},
    switchMonitorCond: function(monitorCond)
    {
        var metric = jQuery("#__th_metric_sel").val();
        var cond = jQuery("#__th_cond_sel").val();
        var metric_info = JKB.cache.th_define[metric];
        var cond_info = metric_info[cond];
        var th_dev_sel_html = '';
        var monitorCond = jQuery("#__th_monitor_set").val();
        
        if(cond_info['dev'] == undefined) 
            return false;

        if( monitorCond == 1 )
        {
            if( JKB.cache.th_dev_count != undefined && JKB.cache.th_dev_count > 0 )
                th_dev_sel_html = '<input style="width:56px;" id="__th_monitor_count" type="text" name="th_monitor_count" value="'+JKB.cache.th_dev_count+'" /> 个';
            else
                th_dev_sel_html = '<input style="width:56px;" id="__th_monitor_count" type="text" name="th_monitor_count" value="" /> 个';
        }
        else if( monitorCond == 2 )
        {
            if( cond_info['dev'].length > 0 || cond_info['dev'].length == undefined )
            {
                for (var devid in cond_info['dev'])
                {

                    th_dev_sel_html += '<div style="width:150px;float:left">';
                    if( JKB.cache.th_dev_name != undefined ) 
                    {
                        var th_dev_name = json_parse(JKB.cache.th_dev_name);
                        if( jQuery.inArray(devid, th_dev_name) != -1 )
                            th_dev_sel_html += '<input type="checkbox" style="position:relative;top:3px;" name="th_dev_sel[]" value="' + devid + '" checked/> '+ cond_info['dev'][devid];
                        else
                            th_dev_sel_html += '<input type="checkbox" style="position:relative;top:3px;" name="th_dev_sel[]" value="' + devid + '" /> '+ cond_info['dev'][devid];
                    }
                    else
                        th_dev_sel_html += '<input type="checkbox" style="position:relative;top:3px;" name="th_dev_sel[]" value="' + devid + '" /> '+ cond_info['dev'][devid];
                    th_dev_sel_html += '</div>';
        ...
        jQuery("#__th_dev_sel").html(th_dev_sel_html);
    },
	showNewMsgSum: function()
	{
		...
		jQuery.getJSON(getRandomUrl(ajax_url), function(json)
		{
			...
			if (msg_sum > 0)
			{
				...
				//jQuery('#__notices').addClass('open');
				jQuery('#__tips').addClass('warn_message');
				jQuery('#__adm_msg_newsum').html(msg_sum_str);
	...
	loadSysInfo: function()
	{
		...
		if(jQuery('#sys_info_include').prop('checked') == false){
    ...
    showDeleteTaskMessage: function()
    {
        this.showAjaxLoader();
        var key = 'show_delete';
        var include_status = 'include';

        if(jQuery('#delete_task_include').prop('checked') == false){
            include_status = 'no_include';
        }
        JKB.loader.setCookie(key, include_status);
    ...
    showDeleteTaskFault: function()
    {
        this.showAjaxLoader();
        var key = 'show_delete_fault';
        var include_status = 'include';
	...
	loadUserViewList: function(view_id)
	{
		...
		jQuery('#__view_list').load(getRandomUrl(ajax_url));
		return true;
	},
     loadShareToUserViewList: function(view_id)
     {
         jQuery('#__share_view_list').html('<div class="menu_title2"><img src="/images/ajax_loader4.gif" /></div>');
         var ajax_url = getAjaxWrapper('get_share_to_user_view_list', {view_id:view_id});
         jQuery('#__share_view_list').load(getRandomUrl(ajax_url));
         return true;
     },
	delUserView: function(view_id)
	{
		...
		__hidden_call.window.location = '/jkb/view_dispose/view_del/s?view_id=' + view_id;
		return true;
	},
	delTaskClass: function(class_id)
	{
		...
		__hidden_call.window.location = '/jkb/task_task_dispose/task_class_del/s?class_id=' + class_id;
		return true;
	},
    ...
    batchDelTaskClass:function(){
        ...
        jQuery('#__task_class_sel').html('<img src="/images/ajax_loader4.gif" />');
        jQuery.post(getAjaxWrapper('add_multi_task_to_class'), {ids:ids, class_id:0}, function()
        {
            jQuery('#__task_class_sel').html('');
            showSuccMsg('分类取消成功！', '', '', {});
        });
	...
	getCheckedTasks: function(obj)
	{
		var err = '很抱歉，请选择至少1个监控项目';
		if(!obj) obj = $('form input[name=task_comp]');
		if (!obj)
		{
			//alert(err);
			return false;
...
//                alert(err);
	...
	loadBatchSettingTree: function(task_sort)
	{
		JKB.submit.ajax_load('','tree/batch_setting_load','loadBatchSetGroupTree',{task_sort:task_sort},function(html){
		   JKB.loader.showAjaxLoader();
		   jkbAlert(html);
		   JKB.loader.hideAjaxLoader();
		});
	},
	BatchSettingApply: function(task_sort)
	{
		if(!JKB.cache.from_task) return false;
		var from_task = JKB.cache.from_task;
		if(!JKB.cache.apply_type) return false;
		var apply_type = JKB.cache.apply_type;
		var task_ids = JKB.loader.getCheckedTasks(document.__batch_form.task_comp);
		if(!task_ids || task_ids == '') return false;
		var action = '';
		switch(apply_type)
		{
		   case 'alert': action = "AlertBatchApply";break;
		   case 'threshold': action = "ThresholdBatchApply";break;
		   case 'monitor': action = "MonitorBatchApply";break;
		   case 'permission': action = "TaskPermissionApply";break;
		   default : break;
		}
		JKB.submit.ajax_post('tree/batch_setting_dispose',action,{task_sort:task_sort,from_task:from_task,task_ids:task_ids},function(result){
			 showSuccMsg('批量设置成功！', '', '', {});
			 $.alerts._jkb_hide();
		});
	  return false;
	},
	loadEntGroup: function(node_type)
	{
		//JKB.loader.showAjaxLoader();
		JKB.submit.ajax_load('ent_task_group','tree/ent_task_group_load','loadTaskGroups',{node_type:node_type});
		//JKB.loader.hideAjaxLoader();
	},
	showEntGroup: function(node_type)
	{
	    if(!node_type){alert('数据错误！');return false;};
	    if(node_type == '6'){//获取API的任务id
	    	var task_ids = $("#api_list_task_ids").val();
	    }else{
	    	var task_ids = JKB.loader.getCheckedTasks();
	    }
	    if(!task_ids || task_ids == ''){
            showSuccMsg('请至少选择一个监控项目！', '', '', {});
            return false;
        }
        console.log(node_type);
		JKB.submit.ajax_load('','tree/ent_task_group_load','loadTaskGroupsCheckbox',{node_type:node_type,task_ids:task_ids},function(html){
		   JKB.loader.showAjaxLoader();
		   jkbAlert(html);
		   JKB.loader.hideAjaxLoader();
		});
	},
	showMyGroup: function(node_type)
	{
		if(!node_type){alert('数据错误！');return false;};
	    if(node_type == '6'){
	    	var task_ids = $("#api_list_task_ids").val();
		...
		if(!task_ids || task_ids == '') return false;
		JKB.submit.ajax_load('','tree','loadMyselfGroupCheckbox',{node_type:node_type,task_ids:task_ids},function(html){
		   JKB.loader.showAjaxLoader();
		   jkbAlert(html);
		   JKB.loader.hideAjaxLoader();
		});
	},
	delEntTaskGroup: function(node_id,node_type,task_id)
	{  
		if(!node_id || !task_id){alert('数据错误！');return false;}
		JKB.submit.ajax_post('tree/ent_task_group_dispose','delTreeLeaf',{node_id:node_id,task_id:task_id},function(result){
			if(!result)
			{  
			   var str = "#__task_";
			   if(node_type == '1'){str = '#host_list_tr_';}else if(node_type == '0'){str ="#__task_name_"}
			   $(str + task_id).remove(); 
			   JKB.loader.loadEntGroup(node_type);
			}
			else
			{
			   showErrorTips(result,'');return false;
			} 
		});
	},
	delMyselfTaskGroup: function(node_id,node_type,task_id)
	{
		if(!node_id || !node_type || !task_id) return false;
		JKB.submit.ajax_post('tree','delTreeLeaf',{task_id:task_id,node_id:node_id,node_type:node_type},function(result){
			if(!result){showErrorTips('删除分组失败','');return false}
			var id_str = "#__task_";
			if(node_type == 'h') id_str = "#host_list_tr_";
			$(id_str + task_id).remove();
			JKB.submit.ajax_load('myself_group','tree','loadMyselfGroup',{type:node_type});
		});
	},
	batchDelEntTaskGroup: function(node_type)
	{   
		 if(node_type == '6'){//获取API的任务id
		    var task_ids = $("#api_list_task_ids").val();
		 }else{
		...
		for(i=0;i<ids.length;i++)
		{
		   ...
		   {  
		      var task_id = ids[i];
		      var node_id = $('#task_comp_' + task_id).attr('ent_node_id');
			  JKB.loader.delEntTaskGroup(node_id,node_type,task_id);
		   }
		}
	},
	batchTaskMediumBindIcon: function(id)
	{
		jQuery('#batch_email').html('<img src="/images/tick.png" />');
	},
	checkShowMediumBind: function(type, medium_array, alert_user_id)
	{
		var task_type  = new Array("email","sms","gtalk","msn","remind");
		var count_alert_settings  =  new Array();
		var count_task_ids = new Array();
		for (var item in task_type)
		{
			count_alert_settings[task_type[item]] = new Array();
			count_task_ids[task_type[item]] = new Array();
		}

		for(var task_id in medium_array){
			if(!isNaN(task_id))
			{
				for(var alert_type in medium_array[task_id])
				{
					if(alert_type != 'remove')
					{
						count_alert_settings[alert_type].push(medium_array[task_id][alert_type]);
						count_task_ids[alert_type].push(task_id);
					}
				}
			}
		}
		...
		{
			if(JKB.loader.checkParamInArray('0', count_alert_settings[task_type[item]]))
			{
				JKB.loader.showBatchTaskMediumBindIcon(type, task_type[item], 0, count_task_ids[task_type[item]], alert_user_id);
			}
			else
			{
				JKB.loader.showBatchTaskMediumBindIcon(type, task_type[item], 1, count_task_ids[task_type[item]], alert_user_id);
			}
		}
	},
	checkParamInArray: function (needle, haystack) {
	...
	showBatchTaskMediumBindIcon: function(task_type, alert_type, stat, task_ids, alert_user_id)
	{
		if (stat == '1')
		{
			jQuery('#' + task_type + '_batch_' + alert_type).html('<a href="" onclick="JKB.loader.batchChangeTaskMediumSettings(\'' + task_type + '\', \'' + alert_type + '\', 0, \'' + task_ids + '\', \'' + alert_user_id + '\');return false;" title="批量禁用通知"><img src="/images/tick.png" /></a>');
		}
		else
		{
			jQuery('#' + task_type + '_batch_' + alert_type).html('<a href="" onclick="JKB.loader.batchChangeTaskMediumSettings(\'' + task_type + '\', \'' + alert_type + '\', 1, \'' + task_ids + '\', \'' + alert_user_id + '\');return false;" title="批量开启通知"><img src="/images/tick_disable.gif" /></a>');
		}
	},
	batchChangeTaskMediumSettings: function(task_type, alert_type, stat, task_ids, alert_user_id)
	{
		this.showAjaxLoader();
		var ajax_url = getAjaxWrapper('batch_task_medium_settings', {task_type:task_type,alert_type:alert_type,stat:stat,task_ids:task_ids,alert_user_id:alert_user_id});

        var ajax_url = getAjaxWrapper('batch_task_medium_settings', {});
        var ajax_data = {task_type:task_type,alert_type:alert_type,stat:stat,task_ids:task_ids,alert_user_id:alert_user_id};

        //jQuery.getJSON(getRandomUrl(ajax_url), function(json)
        jQuery.post(getRandomUrl(ajax_url), ajax_data, function(json)
        {
			//stat = json['stat'];
            stat = JSON.parse(json).stat;
			task_ids = task_ids.split(",");
			JKB.loader.showBatchTaskMediumBindIcon(task_type, alert_type, stat, task_ids, alert_user_id);
			for(var item in task_ids)
			{
				if(!isNaN(task_ids[item]))
				{
					if(task_type == 'site')
					{
						JKB.loader.showTaskMediumBindIcon(alert_type, stat, task_ids[item], alert_user_id);
					}
					else if(task_type == 'server')
					{
						JKB.loader.showServerMediumBindIcon(alert_type, stat, task_ids[item], alert_user_id);
					}
					else if(task_type == 'service')
					{
						JKB.loader.showServiceMediumBindIcon(alert_type, stat, task_ids[item], alert_user_id);
					}
					else if(task_type == 'custom')
					{
						JKB.loader.showCustomMediumBindIcon(alert_type, stat, task_ids[item], alert_user_id);
					}
					else if(task_type == 'content')
					{
						JKB.loader.showContentMediumBindIcon(alert_type, stat, task_ids[item], alert_user_id);
					}
				}
			}
		});
	...
	showContentMediumBindIcon: function(medium, stat, task_id, user_id)
	{		
		var medium_id = 'content_' + medium;
		if (stat == '1')
		{
			jQuery('#__' + medium_id + '_' + task_id).html('<a href="" onclick="return JKB.loader.changeContentMediumSettings(\'' + medium + '\', \'' + task_id + '\', \'' + user_id + '\');" title="禁用通知"><img src="/images/tick.png" /></a>');
		}
		else
		{
			jQuery('#__' + medium_id + '_' + task_id).html('<a href="" onclick="return JKB.loader.changeContentMediumSettings(\'' + medium + '\', \'' + task_id + '\', \'' + user_id + '\');" title="开启通知"><img src="/images/tick_disable.gif" /></a>');
		}
	},
	changeContentMediumSettings: function(medium, task_id, user_id)
	{
		var medium_id = 'content_' + medium;
		jQuery('#__' + medium_id + '_' + task_id).html('<img src="/images/loading.gif" />');
		jQuery.getJSON('/jkb/content_dispose/modifyTaskAlertSetting/x',{task_id:task_id,medium:medium,u:user_id},function(json){
			stat = json['stat'];
			JKB.loader.showContentMediumBindIcon(medium, stat, task_id, user_id);
		});
		return false;
	},
	
	showCustomMediumBindIcon: function(medium, stat, task_id, user_id)
	{
		var medium_id = 'custom_' + medium;
		if (stat == '1')
		{
			jQuery('#__' + medium_id + '_' + task_id).html('<a href="" onclick="return JKB.loader.changeCustomMediumSettings(\'' + medium + '\', \'' + task_id + '\', \'' + user_id + '\');" title="禁用通知"><img src="/images/tick.png" /></a>');
		}
		else
		{
			jQuery('#__' + medium_id + '_' + task_id).html('<a href="" onclick="return JKB.loader.changeCustomMediumSettings(\'' + medium + '\', \'' + task_id + '\', \'' + user_id + '\');" title="开启通知"><img src="/images/tick_disable.gif" /></a>');
		}
	},
	changeCustomMediumSettings: function(medium, task_id, user_id)
	{
		var medium_id = 'custom_' + medium;
		jQuery('#__' + medium_id + '_' + task_id).html('<img src="/images/loading.gif" />');
		var ajax_url = getAjaxWrapper('custom_medium_settings', {task_id:task_id,medium:medium,u:user_id});
		jQuery.getJSON(getRandomUrl(ajax_url), function(json)
		{
			stat = json['stat'];
			JKB.loader.showCustomMediumBindIcon(medium, stat, task_id, user_id);
		});
		return false;
	},
	...
	hideTask: function(task_id,status)
	{
		jQuery('#__task_' + task_id).fadeOut('slow');
        jQuery('#__task_name_' + task_id).fadeOut('slow', function(){
            jQuery(this).empty();
        });
		jQuery('#host_list_tr_' + task_id).fadeOut('slow');
		//全景处理
		if($('#task_num'))
		{ 
		    var task_sum = parseInt($('#task_num span:eq(0)').html());
			$('#task_num span:eq(0)').html(task_sum - 1);
			if(status == 3)
			{
				var stop_num = $('#task_num span:eq(1)').html();
				$('#task_num span:eq(1)').html(stop_num - 1);
			}
			if(status == 1)
			{
				var open_num = $('#task_num span:eq(2)').html();
				$('#task_num span:eq(2)').html(open_num - 1);
			}
		}
	},
	setMetricSelectorTab: function(tab)
	{
		...
		{
			jQuery('#' + JKB.cache.curr_metric_tab).removeClass("active");
		}
	...
	{
        var sel_details = $("#sel_details");
        var button = $("#addWidgetButton");
        if( sel_details.is(":hidden") == false ) {
            button.removeClass("minus_icon").addClass("add_icon");
            sel_details.slideUp();
            return true;
        }
        button.removeClass("add_icon").addClass("minus_icon");

		JKB.cache.curr_overlay_container = elem;
		sel_details.html('<img src="/images/loading.gif" />');
		var ajax_url = getAjaxWrapper('get_metric_selector', {});
		sel_details.load(getRandomUrl(ajax_url), function()
		{
			sel_details.slideDown();
	...
	loadMetricSiteTaskMetric: function(view_id, task_type, task_id, task_name, dom)
	{
		JKB.cache.metric_task_name = task_name;
        var metric_list = $("#group_final");
        $(dom).parent("li").addClass("active").siblings("li").removeClass("active");

        metric_list.html('<div style="padding:6px 15px;"><img src="/images/loading.gif" /></div>');
        var ajax_url = getAjaxWrapper('get_metric_site_task_metric', {view_id:view_id, task_type:task_type, task_id:task_id});
        metric_list.load(getRandomUrl(ajax_url), function()
        {
            $("#metric_task_list").css("width", "59%");
            $("#server_metric_list").hide();
            metric_list.fadeIn();
        });
	},
	...
	loadMetricServerTask: function(server_id, server_name, view_id, dom)
	{
        $("#group_final").html('');
		JKB.cache.metric_task_name = server_name;
        var metric_list = $("#server_metric_list");
        $(dom).parent("li").addClass("active").siblings("li").removeClass("active");
		...
		metric_list.load(getRandomUrl(ajax_url), function()
		{
            $("#metric_task_list").css("width", "47%");
            metric_list.css("width", "12%").fadeIn();
            metric_list.fadeIn();
		});
	},
	...
	loadMetricServiceTaskMetric: function(view_id, task_type, task_id, task_name, dom)
	{
        JKB.cache.metric_task_name = task_name;
        var metric_list = $("#group_final");
        $(dom).parent("li").addClass("active").siblings("li").removeClass("active");
	...
	addWidget: function(elm, view_id, widget_type, widget_title, task_id, task_type, task_sort, task_dev, period, range)
	{
		...
		jQuery.getJSON(getRandomUrl(url), function(json) {
            ...
            if ( res == '1') { // failed
				...
				//showSuccMsg('添加成功！', '', 'msg_container_view_widget', {});
				jQuery(elm).removeClass();
				jQuery(elm).addClass('ok_icon2').parent("span").addClass("disabled");
				jQuery(elm).attr('is_selected','1');
	...
	renderOneWidget: function(view_id, widget_id, period, range, col_to_add)
	{
		...
		var id = col_to_add + 1;
		if (append) {
			$('#__wids_col_' + id).append('<li id="widget_' + widget_id + '">');
		} else {
			g_layout[col_to_add].unshift('widget_' + widget_id); // 新增 widget
			$('#__wids_col_' + id).prepend('<li id="widget_' + widget_id + '"">');
		}
	...
	addPurchaseItemToCart: function(item_type, calculator, item_num)
	{
		...
		ajax_url = getAjaxWrapper(ajax_func, {item_type:item_type, item_num:item_num});
		...
		else
		{
			...
			switch (elem)
			{
				...
				case '__query_result':
					...
					break;
				case '__dropmenu_ztree':
				    JKB.loader.loadZtree(param);
				    break;
				case '__dropmenu_api_selector':
					JKB.loader.loadDropApiSelector(param);
					break;	
			}
		}
	},
	...
	loadDropApiSelector: function(param)
	{
		var json_date_range = param['json_date_range'];
		jQuery('#__dropmenu_api_selector').html('<span><img style="margin:0 10px;" src="/images/ajax_loader4.gif"></span> 正在加载...');
		var ajax_url = '/jkb/api_load/load_selector_list/x';
		jQuery('#__dropmenu_api_selector').load(ajax_url,{'json_date_range':json_date_range} ,function()
		{
	...
	//加载ztree
	loadZtree: function(param)
	{
	  var node_type = param['node_type'];
	  var task_id = param['task_id'];
	  var task_type = param['task_type'];
	  var host_id = param['host_id'];
	  jQuery('#__load_ztree').html('<span style="padding-left:380px;"><img src="/images/ajax_loader4.gif"></span> 正在加载...');
	  var ajax_url = getAjaxWrapper('load_ztree',{node_type:node_type,task_id:task_id,task_type:task_type,host_id:host_id});
	  jQuery('#__load_ztree').load(getRandomUrl(ajax_url),function(result){
	  		if(!result)
			{
				var html = '<li style="text-align:center;">您还没有创建任务分组!</li>';
				jQuery('#__load_ztree').html(html);
			}
	  });
	},
	...
	{
		JKB.submit.ajax_post('task/task_dispose','switch_task_pause', {task_id:task_id}, function(data)
		{
			if (data == '1')
			{
				if(task_summary)
	...
	switchServerPause: function(server_id)
	{   
		JKB.submit.ajax_post('task/server_dispose','switch_server_pause',{server_id:server_id},function(data){
			switch(data)
			{
			    case '-2':
			        showErrorTips('很抱歉，该监控项目无法开启，您可以尝试降低监控频率后重新开启。', '');break;
			    case '-1':
			        showErrorTips('很抱歉，该监控项目无法开启，您创建的监控项目数量已达到上限。', '');break;
				case '0':
				    showErrorTips('很抱歉，该监控项目开启或暂停失败。', '');break;
				case '1':
					jQuery('#server_pause_'+server_id).show();
					jQuery('#server_open_'+server_id).hide();
				    showSuccMsg('已开启服务器，同时服务器SNMP监控任务也已开启，告警通知也将开启工作。', '', '', {});break;
				case '3':
				    jQuery('#server_open_'+server_id).show();
					jQuery('#server_pause_'+server_id).hide();
				    showSuccMsg('已暂停服务器，同时服务器SNMP监控任务也已暂停，您将无法收到告警通知。', '', '', {});break;	
				default:break;					
			}
		});
	},
	switchBatchServerPause: function(obj,type)
	{
		if(!obj)
		{
			jkbAlert(err, 'error');
		...
		this.showAjaxLoader();
		task_ids = JKB.loader.getCheckedTasks();
		server_ids = task_ids.replace(/-s/g,'');
		JKB.submit.ajax_post('task/server_dispose','switch_batch_server_pause',{server_ids:server_ids,type:type},function(data){
			JKB.loader.hideAjaxLoader();
		    switch(data)
			{
			   case '-2':
			      showErrorTips('很抱歉，监控项目已无法开启，您可以尝试降低监控频率后重新开启。', '');break;
			   case '-1':
			      showErrorTips('很抱歉，监控项目无法开启，您创建的监控项目数量已达到上限。', '');break;
			   case '1':
			      split_server_id = server_ids.split(",");
				  for(i=0;i<split_server_id.length;i++)
				  {
					if(split_server_id[i] != "")
					{
						jQuery('#server_pause_' + split_server_id[i]).show();
						jQuery('#server_open_' + split_server_id[i]).hide();
					}
				  }
				  showSuccMsg('已开启监控，告警通知也将开启工作', '', '', {});
				  break;
			   case '3':
				  ...
				  for(i=0;i<split_server_id.length;i++)
				  {
					if(split_server_id[i] != "")
					{
						jQuery('#server_pause_' + split_server_id[i]).hide();
						jQuery('#server_open_' + split_server_id[i]).show();
					}
				  }
			      showSuccMsg('已暂停监控，您将无法收到告警通知', '', '', {});
			      break;
	...
	switchBatchServerDelete: function(obj)
	{
		if (!confirm('确定要删除所选项目吗？'))
		{
			return false;
		...
		task_ids = JKB.loader.getCheckedTasks();
		server_ids = task_ids.replace(/-s/g,'');
		JKB.submit.ajax_post('task/server_dispose','switch_batch_server_del',{server_ids:server_ids},function(data){
		  if(data == '1')
		  { 
		    showSuccMsg('已删除所选监控项目!', '', '', {}); 
			split_server_id = server_ids.split(",");
			for(i=0;i<split_server_id.length;i++){
				if(split_server_id[i] != "")
				{
					parent.JKB.loader.hideTask(split_server_id[i]);
				}
			}
		  }
		  else
		  {
			showErrorTips('很抱歉,删除监控项目失败!', '');
		  }
		});
	},
	switchServerTaskPause: function(task_id)
	{    
		JKB.submit.ajax_post('task/server_dispose','switch_server_task_pause', {task_id:task_id}, function(data)
		{
			if (data == '1')
			{
				jQuery('#__task_pause_' + task_id).html('暂停监控');
			...
			{
			   showErrorTips('服务器没有开启，请先开启服务器！','');
			}
			else if (data == '-1')
			{
				showErrorTips('很抱歉，该监控项目无法开启，您可以创建的监控项目数量已达到上限。', '');
			}else if (data == '-2'){
				showErrorTips('此插件已经停止使用！请确保插件正在使用！' , '');
			}else if (data == '-3'){
				showErrorTips('没有操作权限！' , '');
			}else if (data == '-4'){
				showErrorTips('修改状态失败！' , '');
			}
		});
	},
	switchServiceTaskPause: function(task_id)
	{
		JKB.submit.ajax_post('task/service_dispose','switch_service_task_pause',{task_id:task_id}, function(data)
		{   
			if (data == '1')
			{
				jQuery('#__task_pause_' + task_id).show();
	...
	switchBatchServiceTaskPause: function(obj, type)
	{
		...
		JKB.submit.ajax_post('task/service_dispose','switch_batch_service_task_pause',{task_ids:task_ids, type:type},function(data)
		{
			if (data == '1')
			{
				JKB.loader.hideAjaxLoader();
	...
	serviceBatchTaskDelete: function(obj)
	{
		...
		JKB.submit.ajax_post('task/service_dispose','service_batch_task_delete', {task_ids:task_ids}, function(data)
		{
			if(data)
			{
				showSuccMsg('已删除所选监控项目！', '', '', {});
	...
	switchCustomTaskPause: function(task_id, list)
	{
		jQuery.post(getAjaxWrapper('switch_custom_task_pause'), {task_id:task_id}, function(data)
		{
			if (data == '1')
			{
                var switchHtml = ( list == true ) ? '<i class="table_handle_icon pause"></i>': '暂停监控';
				jQuery('#__task_pause_' + task_id).html(switchHtml);
				jQuery('#__task_pause_status_' + task_id).html('');
				showSuccMsg('已开启监控，告警通知也将开启工作', '', '', {});
			}
			else if (data == '3')
			{
                var switchHtml = ( list == true ) ? '<i class="table_handle_icon start"></i>': '开启监控';
				jQuery('#__task_pause_' + task_id).html(switchHtml);
				jQuery('#__task_pause_status_' + task_id).html('已暂停监控');
				showSuccMsg('已暂停监控，您将无法收到告警通知', '', '', {});
			}
			else if (data == '-1')
			{
				...
				showSuccMsg('此插件已经停止使用！请确保插件正在使用！' , '', '', {});
			}else if (data == '-3'){
				showSuccMsg('没有操作权限！' , '', '', {});
			}else if (data == '-4'){
				showSuccMsg('修改状态失败！' , '', '', {});
	...
	editServerFreq: function(elem,server_id)
	{
		JKB.loader.showPopLayer(elem, 'server_service_freq_edit', {task_id:server_id,task_style:'server'});
	},
	editServiceFreq: function(elem,task_id)
	{
		JKB.loader.showPopLayer(elem, 'server_service_freq_edit', {task_id:task_id,task_style:'service'});
	},
	taskFreqUpdate: function(task_id,task_style)
	{
		var freqs = document['__'+task_style+'_freq_edit_form_' + task_id]['frequency_' + task_id];
		var sel_freq = 0;
		if (freqs.length)
		{
		...
		if(sel_freq == 0) {
			jQuery('#__'+task_style+'_freq_edit_' + task_id).qtip('destroy');
			return false;
		}
		
		jQuery('#__'+task_style+'_freq_' + task_id).html('<img src="/images/loading.gif" />');
		JKB.submit.ajax_post('task/'+ task_style +'_dispose',task_style+'_freq_update',{task_id:task_id,freq:sel_freq}, function(data)
		{   
			if (data == '1')
			{
				jQuery('#__'+task_style+'_freq_edit_' + task_id).qtip('destroy');
			...
			{
				jQuery('#__'+task_style+'_freq_edit_error_' + task_id).css('display', 'block');
				jQuery('#__'+task_style+'_freq_edit_error_' + task_id).html('请选择正确的监控频率！');
			}
			
			jQuery('#__'+task_style+'_freq_' + task_id).html(sel_freq + '分钟');
		});
	},
	...
	displayAnalogUrl: function()
	{
		...
		callback_url = callback_url.replace(reg,"|");
	...
	loadAddToView: function(task_id, task_type, task_sort, widget_type, task_dev, display_id)
	{
		...
		jQuery('#div_add_to_view_' + display_id).show().html('<img src="/images/ajax_loader2.gif" />');
		jQuery('#div_add_to_view_' + display_id).load(getRandomUrl(ajax_url), function()
		{
		});
	},
	closeServerDiskShow: function(task_id,disk_id)
	{   
	    JKB.submit.ajax_post('task/server_dispose','close_server_disk_show',{task_id:task_id,disk_id:disk_id},function(data){
			var disk = disk_id.replace(/\//g,'');
			if(data == '1')
			{ 
			   $('#' + task_id + '-' + disk).hide('slow');
			   $('#' + task_id + '-' + disk + '-close').hide();
			   $('#' + task_id + '-' + disk + '-open').show();
			} 
			if(data == '-2'){alert('您无权限操作！')}
		});
	},
	openServerDiskShow: function(task_id,disk_id)
	{
		 JKB.submit.ajax_post('task/server_dispose','open_server_disk_show',{task_id:task_id,disk_id:disk_id},function(data){
			var disk = disk_id.replace(/\//g,'');
			if(data == '1')
			{ 
			   $('#' + task_id + '-' + disk).show();
			   $('#' + task_id + '-' + disk + '-open').hide();
			   $('#' + task_id + '-' + disk + '-close').show();
			} 
			if(data == '-2'){alert('您无权限操作！')}
		});
	},
	...
	loadUserAlertHistory: function(select_type)
	{
		this.showAjaxLoader();
		var uid = '';
		if(jQuery('#__alert_user_single').prop('checked') == true || select_type){
			uid = jQuery('#__share_user_list').val();
		}
	...
	loadServerListStatus: function(period, scale,page)
	{
		var period;
		var range;
		var scale;
		...
		var ent_node_id = JKB.cache.ent_node_id;
		if(view_type == 'list')
		{
			...
			var ajax_url = '/ajax_server_list_status.php?type=' + status_type + '&node_type=' + node_type + '&n_id=' + n_id + '&ent_node_id=' + ent_node_id;
		...
		else if(view_type == 'pic')
		{
			...
			var ajax_url = '/ajax_server_view.php?n_id=' + n_id + '&dev_type=' + status_type + '&ent_node_id=' + ent_node_id;
        ...
        if(page && page>=1) ajax_url +="&page=" + page;
        if(JKB.cache.keyword != undefined) ajax_url +="&keyword=" + JKB.cache.keyword;
		JKB.loader.showAjaxLoader();
	...
	{		
		var excel_url = '/excel_task_list.php?period=' + JKB.cache.curr_period + '&range=' + JKB.cache.date_range + '&keyword=' + JKB.cache.s + '&status=' + JKB.cache.search_status;
	...
	{		
		...
		jConfirm(tree_html, '导出故障历史', function(r) {
			if(r){
				...
				var excel_url = '/excel_fault_list_ing.php?period=' + selected_fault_time;
				window.location.href = excel_url;
				return true;
			}
		});
	...
	{		
		...
		if(JKB.cache.ent_node_id) var ent_node_id = JKB.cache.ent_node_id;
		if(ent_node_id)
		{
		    var ent_node_type = JKB.cache.ent_node_type;
			switch(ent_node_type)
			{
			    case '0':
				   JKB.loader.loadTaskListPage();break;
				case '1':
				   loadListData('server_list_status', '/ajax_server_list_status.php?type=list&ent_node_id='+ent_node_id);break;
				case '2':
				   JKB.submit.ajax_load('__service_task_list','task','load_service_task',{ent_node_id:ent_node_id});break;
				case '3':break;
				case '6':JKB.loader.loadApiTaskList();break;
	            default:break;
			}
		}
		else
	...
	editMonitorGroup: function(group_id, type)
	{
		if (type == 'del'){
			if (confirm('您确定要删除分组吗？ 删除分组后分组中的监控项目将使用默认监测点分组')){
				...
				JKB.submit.go_dispose('account/monitor_dispose','monitor_group_del','group_id='+group_id);
				return false;
			}else {
				return false;
			}
	...
	loadUserNoticeNum: function(no_load)
	{
		...
		jQuery.get(getRandomUrl(ajax_url), function(notice_num)
		{
			...
			{
				jQuery("#__user_notice_num").html('0'); 
			}
		});		
	},
	expendNotice: function(uniq_id)
	{
		...
		var title = jQuery("#__title_" + uniq_id);
		...
		if (row.hasClass('notice_expand')) {	
            ...
            jQuery("#__title_" + uniq_id + " p").css("height","30px");
		}else {	
            ...
            jQuery("#__title_" + uniq_id + " p").css("height","auto");
		}
	},
	...
	{
		var ajax_url = getAjaxWrapper('get_dns_domain_input');
		var sum_ip_num = $("input#metric_domain").length;
		if(sum_ip_num >= 3){
			jQuery('#msg_domain_error').html("抱歉,域名最多添加三个指标").show("slow");
			setTimeout(function(){jQuery('#msg_domain_error').hide("slow");}, 3000);
		}else{
			jQuery.get(getRandomUrl(ajax_url), function(data){
				jQuery('#domain_metric_table tbody').append(data);
			}); 
		}
	},
	deleteDnsMetricInput: function(task_id, metric_id, metric_type)
	{
		...
		if(task_id){
			...
			else if(metric_type == 'domain')
			{
				var ajax_url = getAjaxWrapper('delete_dns_domain_metric', {task_id:task_id, metric_id:metric_id});
				jQuery.get(getRandomUrl(ajax_url), function(data){
					if(data == '1'){
						jQuery('#exist_domain_' + metric_id).remove();
					}
				}); 
			}
		}else {
	...
	{
		var use_nat = jQuery('#use_agent')[0].checked;
		if ( use_nat )
		{
			jQuery('#agentList').fadeIn();
			jQuery('#agentList input:radio').attr("checked",false);
		}
		else
		{
			jQuery('#agentList').fadeOut();
			jQuery('#server_ip').attr("readonly",false);
			jQuery('#snmp_port').attr("readonly",false);
			jQuery('#snmp_comm').attr("readonly",false);
			jQuery('#snmp_user').attr("readonly",false);
			jQuery('#snmp_pass').attr("readonly",false);
			jQuery('#snmp_authtype_1').attr("contenteditable",false);
			jQuery('#snmp_authtype_0').attr("contenteditable",false);
			jQuery('#snmp_v_1').attr("contenteditable",false);
			jQuery('#snmp_v_0').attr("contenteditable",false);
			jQuery('#server_systype_0').attr("contenteditable",false);
			jQuery('#server_systype_1').attr("contenteditable",false);
		}
	},
	showPlugList: function(id){
		if(jQuery('.__plug_'+id).css("display")=='none'){
			jQuery('.__plug_'+id).fadeIn();
		}else{
			jQuery('.__plug_'+id).fadeOut();
		}
	},
	reflashAgnetList: function(time){
		var ajax_url = getAjaxWrapper('reflesh_agent', {time:time});
		jQuery.get(getRandomUrl(ajax_url), function(data){
			if(data == '1'){
	...
	showDownload: function(id){
		jQuery('#agentDown_'+id).hide();
		jQuery('#agentDownImg_'+id).show();
	},
	talbeCssOn: function(obj){
		jQuery(obj).css('backgroundColor', ' #e9e5e5');
	},
	talbeCssOut: function(obj,color){
		jQuery(obj).css('backgroundColor', color);
	},
	showErrorInfo: function(obj,id){
		top=jQuery(obj).offset().top;
		left=jQuery(obj).offset().left;
		jQuery('#error_info_'+id).css({"left":left+15,"top":top});

		jQuery('#error_info_'+id).show();
	},
	hideErrorInfo: function(obj,id){
		jQuery('#error_info_'+id).hide();
	}
};
...
{
	if($('#use_dns_server').attr('checked') == true || $('#use_dns_server').attr('checked')=='checked'){
...
{
	if($('#' + checkbox_id).attr('checked') == true || $('#' + checkbox_id).attr('checked') == 'checked'){
...
{	
	...
	{
		$('#ip_metric_div').show();
		$('#domain_metric_div').hide();
	}
	else if(dns_type == 'NS')
	{
		$('#domain_metric_div').show();
		$('#ip_metric_div').hide();
	}
	else
	{
...
function chooseClass(choose, select_id)
{
    $(choose).addClass("active").siblings("a").removeClass("active");

    JKB.cache.view_type_tab = select_id;
	if(select_id == 'list') JKB.cache.server_tab = 'list';
}
...
var tomcat_shuiming=0;
function show_tomcat_shuiming(){
	if(tomcat_shuiming==0){
		jQuery("#shuoming").show();
		tomcat_shuiming=1;
	}else{
		jQuery("#shuoming").hide();	
		tomcat_shuiming=0;
	}
}
...
function showErrorMsg1(id, msg)
{
	jQuery('#control-group-' + id).attr('class', 'control-group error');	
	jQuery('#help-inline-' + id).html(msg);	
}

function clearErrorMsg1(id)
{
	jQuery('#control-group-' + id).attr('class', 'control-group');	
	jQuery('#help-inline-' + id).html('');	
}
function clearErrorMsg2()
{
	/*
	$.each(JKB.cache.error_fields, function(id, v)
	{
		alert(id);
	});
	*/
	for(var id in JKB.cache.error_fields)
	{
  		clearErrorMsg1(id);
 	}
}
...
/**
 * 云智慧点计算	——站点监控（一个监控项目，一个监测点，一个月的监控次数，60分钟监控频率 == 1个云智慧点）
 * 			——服务器监控（一个监控项目，一个月的监控次数 == 5个云智慧点）
 * 			——服务监控（一个监控项目，一个月的监控次数 == 3个云智慧点）
 * @param {} monitors
 * @param {} site_tasks
 * @param {} server_tasks
 * @param {} service_tasks
 ...
 * @return {}
 */
function calculate_points(monitors, site_tasks, server_tasks, service_tasks, sms, frequency)
{
	var points = {};
	points.site = 0;
	points.server = 0;
	...
	//var server_service_frequency = 5;		//暂时和监控宝现有一致，按5分钟的频率
	var site_times = parseInt(60/parseInt(frequency));		//站点的每月基数
	//var server_times = parseInt(1440/parseInt(server_service_frequency)); //服务器的每月基数
	
	points_site = parseInt(monitors)*parseInt(site_tasks)*site_times;	//站点每月消耗点数
	points_server = parseInt(server_tasks)*5;
	points_service = parseInt(service_tasks)*3;
	
	var every_points = (points_site+points_server+points_service+parseInt(sms))/30;
	every_points = every_points.toFixed(4);
	
	return every_points;
}

var agent_point_id=1;
function add_agent_Point(){
	jQuery('#agentlist').after('<div  id="agentOne_'+agent_point_id+'" ><select name="plugin_task_uselist_id[]"  style="min-width: 130px">'+jQuery('#agentOne').html()+'</select><a href="" title="减少采集器节点" onclick=\'jQuery("#agentOne_'+agent_point_id+'").remove();return false;\'> 删除 </a><br><br></div>');
	agent_point_id=agent_point_id+1;
	return false;
}
...
function amClickedOnBullet(chart_id, graph_index, value, series, url, description){
	...
	if(isday==0){
		...
		datetime=""+sel_date+" "+series+":00"
		var tmp_datetime = datetime.replace(/:/g,'-'); 
		tmp_datetime = tmp_datetime.replace(/ /g,'-'); 
		var arr = tmp_datetime.split("-"); 
		var now = new Date(Date.UTC(arr[0],arr[1]-1,arr[2],arr[3]-8,arr[4],arr[5])); 
		ti=parseInt(now.getTime()/1000);
		$("tr[class^='__hour_']").hide()
		$("a[class^='__title_']").html('展开')
		jQuery(".__hour_"+ti).show()
		$("a[href^='http://m.jiankongbao.com']").focus()
	}else{
		h=location.href;
		re = h.match('([^?])+');
		if(series>100000){
			series=series/1000;
			day=getLocalTime(series);
			location.href=re[0]+'?period=custom&range='+day+','+day+'&scale=hour';
			return false;
		}
		var today=new Date();
		var year = today.getFullYear();
		location.href=re[0]+'?period=custom&range='+year+'-'+series+','+year+'-'+series+'&scale=hour';
	}
} 
...
function listShowSite(id,date){
	if(isday!=0){
		h=location.href;
		re = h.match('([^?])+');
		location.href=re[0]+'?period=custom&range='+date+','+date+'&scale=hour';
		return false;
	}
	if(jQuery('.__hour_'+id).css("display")=='none'){
		jQuery('.__hour_'+id).fadeIn();
		jQuery('.__title_'+id).html('收起');
	}else{
		jQuery('.__hour_'+id).fadeOut();
		jQuery('.__title_'+id).html('展开');
	}
	
}
function changeSiteList(obj){
	var use_a = jQuery(obj)[0].checked;
	if(use_a){
		jQuery('#js_c').fadeIn();
		jQuery('#more_c').hide();
	}else{
...
function tracerouteInit(){
	$(document).ready(function(){
			isAgent = jQuery('#use_agent')[0].checked;
			if(isAgent){
				jQuery('#frequency_2').attr('disabled',false);
				jQuery('#frequency_5').attr('disabled',false);
				jQuery('#frequency_10').attr('disabled',false);
				jQuery('#frequency_5').attr('checked',true);
			}else{
				jQuery('#frequency_2').attr('disabled','disabled');
				jQuery('#frequency_5').attr('disabled','disabled');
				jQuery('#frequency_10').attr('disabled','disabled');
				jQuery('#frequency_15').attr('checked',true);
			}
		});
}
//验证字符串全部由数字组成
...
function changeSQLList(obj,id){
	$("#m_read div").each(function(k, v){
		$(v).css('background-color','#FFFFFF');
	});
	$(obj).css('background-color','#00B6FF');
	$(".clr_oracle").each(function(k, v){
		$(v).hide();
	});
	$("#"+id).show();
}
...
//修改app绑定备注
function updateAppdesc(id,str){
	
	 var str=prompt("请输入备注",str);
	 if(str){
		 location.href='/dispose.php?__action=editApp&id='+id+'&desc='+str;
	 }
}
